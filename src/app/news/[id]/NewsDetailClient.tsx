...
export default function NewsDetailClient({ id, initialNews }: { id: string; initialNews?: INews | null }) {
  const { t, i18n } = useTranslation();
  const isUr = i18n.language === 'ur';

  const [news, setNews] = useState<INews | null>(initialNews || null);
  ...
  const shareTitle = isUr && news.titleUr ? news.titleUr : news.titleEn;
  const shareUrl = absoluteUrl(`/news/${news._id}`);
  ...
  const jsonLdNews = {
    ...
    ...(news.coverImage ? { image: absoluteUrl(news.coverImage.startsWith('/') ? news.coverImage : `/uploads/${news.coverImage}`) } : {}),
  };
    ...
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
      ...
      <article className="bg-white rounded-xl border border-govt-border p-6 md:p-8 shadow-sm space-y-6">
        ...
        <ShareButtons
          url={shareUrl}
          title={shareTitle}
          description={news.bodyEn?.substring(0, 120) || news.category}
        />
      </article>
    </div>
  );
