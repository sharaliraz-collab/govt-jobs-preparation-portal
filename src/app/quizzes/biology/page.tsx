...
const cleanQuestionText = (text: string) => {
  if (!text) return '';
  return text
    .replace(/^(Biology\s+|Cell\s+Biology\s+|Enzymes\s+|Bioenergetics\s+|Genetics\s+)?(Concept\s+)?Q\d+(\s*\([^)]*\))?:\s*/i, '')
    .replace(/^(بيالوجي\s+|سيل\s+بيالوجي\s+)?سوال\s*\d+(\s*\([^)]*\))?:\s*/i, '');
};
...
export default function BiologyQuizPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-8 space-y-6">
      ...
      {/* Top Header Banner */}
      <div className="bg-gradient-to-r from-emerald-950 via-teal-900 to-green-950 text-white p-6 sm:p-8 rounded-3xl shadow-xl border border-emerald-800/40 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="bg-emerald-500/20 border border-emerald-400/40 text-emerald-300 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full">
                Grades 9–12 &amp; Entry Tests (MDCAT • NTS • SPSC)
              </span>
            ...
            <h1 className="text-2xl sm:text-4xl font-black tracking-tight text-white flex items-center gap-3">
              <Dna className="w-8 h-8 text-emerald-400 fill-current animate-pulse" />
              <span>1000 Biology MCQs Solved Bank</span>
            </h1>
            <p className="text-xs sm:text-sm text-emerald-100/90 leading-relaxed max-w-3xl">
              Complete topic-wise solved Biology question bank covering Cell Biology, Biological Molecules, Enzymes, Bioenergetics, Human Physiology, Genetics, Biotechnology, and Ecology.
            </p>
          </div>
