...
const FormCard: React.FC<FormCardProps> = ({ form }) => {
  ...
  const handleDownload = () => {
    ...
    if (form.file.startsWith('/uploads')) {
      window.open(form.file, '_blank');
    } else {
      window.open(`/api/forms/${form._id}/download`, '_blank');
    }
    setTimeout(() => setDownloading(false), 1200);
  };

  return (
    <div className="card-premium border-l-4 border-l-govt-gold/60 p-5 flex flex-col justify-between" id={`form-card-${form._id}`}>
      <div>
