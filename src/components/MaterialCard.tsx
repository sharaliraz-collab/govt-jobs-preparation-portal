...
import { FileDown, BookOpen, Download, Eye } from 'lucide-react';
...
const MaterialCard: React.FC<MaterialCardProps> = ({ material }) => {
  ...
  const handleDownload = () => {
    ...
    if (material.file.startsWith('/uploads') || material.file.startsWith('http')) {
      window.open(material.file, '_blank');
    } else {
      window.open(`/api/materials/${material._id || material.id}/download`, '_blank');
    }
  ...
  return (
    <>
      <div className={`card-premium border-l-4 ${borderClass} p-5 flex flex-col justify-between hover:shadow-md transition`} id={`material-card-${material._id || material.id}`}>
        <div>
        ...
        <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
          ...
          <button
            ...
            id={`download-btn-${material._id || material.id}`}
          >
