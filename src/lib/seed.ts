...
export async function seedAllData() {
  ...
  try {
    const adminEmail = process.env.ADMIN_EMAIL || 'admin@govtjobs.pk';
    ...
    // 3. Seed Questions & Quizzes if empty
    const qCount = await Question.countDocuments();
    if (qCount === 0) {
      const q1 = await (Question as any).create({
      ...
      const q2 = await (Question as any).create({
      ...
      const q3 = await (Question as any).create({
      ...
      const q4 = await (Question as any).create({
    ...
    // 5. Seed Forms if empty
    const formCount = await FormDoc.countDocuments();
    if (formCount === 0) {
      await FormDoc.insertMany([
        {
